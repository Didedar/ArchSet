...
void main() {
  ...
  testWidgets('system back dispatches a single save carrying the audio path, '
      'then pops once the save succeeds', (tester) async {
    ...
    final captured = verify(() => editorBloc.add(captureAny())).captured;
  ...
  testWidgets('a second system back while the first save is in flight does not '
      'dispatch a second save', (tester) async {
    await pumpAndOpenEditor(tester);
  ...
  testWidgets('a save failure keeps the editor open and surfaces a snackbar '
      'instead of hanging', (tester) async {
    await pumpAndOpenEditor(tester);

    await tester.binding.handlePopRoute();
    await tester.pump();

    editorStateController.add(const EditorSaveFailure(message: 'db locked'));
    await tester.pumpAndSettle();
